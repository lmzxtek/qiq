...
(function () {
  ...
  // 删除加载更多按钮
  const delMoreBtn = document.createElement("button");
  delMoreBtn.id = "delMoreBtn";
  delMoreBtn.innerHTML = "📛";
  delMoreBtn.className = "action-btn";
    ...
    async function autoLoadMoreClicker(btn) {
        ...
        try {
            while (isRunning) {
                ...
                const loadMoreBtn = rightpanel.querySelector( '.flex.justify-center.items-center button' );
  ...
  async function triggerLoadMore() {
    const rightpanel = document.querySelector('body > div:first-child > div:first-child > div:last-child');
    const btn = rightpanel.querySelector( '.flex.justify-center.items-center button' );
    if (!btn) {
  ...
  async function delLoadMoreBtn() {
    ...
    try {
      btn.disabled = true;
      // showMessage(" 👉 移除加载更多按钮 ...");
      // btn.remove();
      showMessage(" 👉 变灰加载更多按钮(2.5s) ...");
      await delay(2500); // 给服务器响应时间
  ...
  async function scrapeData(isFiltered = false, isOnlySuper = false) {
      ...
      const rightpanel  = document.querySelector('body > div:first-child > div:first-child').querySelector(':scope > div:last-child');
      // console.info('rightpanel:', rightpanel);
      const rightpanel2 = rightpanel.querySelector(':scope > div:last-child');
      // console.info('rightpanel2:', rightpanel2);
      const rightpanel3 = rightpanel2.querySelector(':scope > div:last-child');
      const cardpanel   = rightpanel3.querySelector('div:nth-of-type(2)');
      const cardBodies  = cardpanel.querySelectorAll( ':scope > div' ); // 获取父元素下的所有直接子 div 元素
      // console.info('cardpanel:', cardpanel);
      ...
      cardBodies.forEach(item => {
          try {
              const card = item.querySelector('div:first-child');
              const subItems = card.querySelectorAll(':scope > div');
              const cardHead = subItems[0].querySelector('div:first-child').querySelector('div:first-child');
              const cardBody = subItems[1].querySelector('div:first-child');
              const cardFoot = subItems[2].querySelector('div:first-child');

              const name = cardHead.querySelector('div:first-child')?.textContent.trim() || 'N/A';

              const detailEle = cardHead.querySelector('div:last-child');
              const location  = detailEle.querySelector('div:last-child')?.textContent.trim() || 'N/A';
              const ipstatus  = detailEle.querySelector('span:first-child')?.textContent.trim() || 'N/A';

              const priceStr  = cardFoot.querySelector('div:first-child')?.textContent.trim() || 'N/A';
              ...
              buyBtn.addEventListener('click', () => {
                const realBtn = cardFoot.querySelector('button');
                if (realBtn) {
                  realBtn.click();
                } else {
              ...
              const infoItems = cardBody.querySelectorAll(':scope > div');
              const node = infoItems[0].querySelector('div:last-child')?.textContent.trim() || 'N/A';  // 节点
              const plan = infoItems[1].querySelector('div:last-child')?.textContent.trim() || 'N/A';  // 套餐
              // const plan = infoItems[1].querySelectorAll(':scope > div')[1]?.textContent.trim() || 'N/A';  // 套餐

              const configEle = infoItems[2].querySelector('div:last-child');  // 配置
              const cpuItems = configEle.querySelector('div:first-child').querySelectorAll(':scope > div');
              const cpuNum  = cpuItems[0]?.textContent.trim() || 'N/A';  // CPU
              const cpuUnit = cpuItems[1]?.textContent.trim() || 'N/A';  // CPU
              let   mem_num  = cpuItems[2]?.textContent.trim() || 'N/A';  // 内存
              const mem_unit = cpuItems[3]?.textContent.trim() || 'N/A';  // 内存

              let mem = mem_num + mem_unit;
              if (mem_unit === 'M') {
                // let mem_num = parseFloat(mem.slice(0, -1));
                if (mem_num>=1024) {
                  mem_num = mem_num/1024;
                  if (mem_num == Math.floor(mem_num)) {
                    mem = mem_num + 'G';
                  }else{
                    mem = mem_num.toFixed(2) + 'G';
                  }
                }
              }

              const diskItems = configEle.querySelector(':scope > div:last-child').querySelectorAll(':scope > div');
              const diskNum  = diskItems[0]?.textContent.trim() ||'N/A';
              const diskUnit = diskItems[1]?.textContent.trim() ||'N/A';

              const networkEle = infoItems[3].querySelector(':scope > div:last-child').querySelector(':scope > div:first-child');  // 网络
              const netStr = networkEle.querySelector(':scope > div:first-child')?.textContent.trim() || 'N/A';
              let bandw   = netStr.split(' ')[1]  || 'N/A' ;
              if (bandw.slice(-1) === 'M') {
              ...
              const usedStr = networkEle.querySelector(':scope > div:last-child')?.textContent.trim() || 'N/A';
              const used    = usedStr.split(' ')[1]  || 'N/A' ;
              const total   = usedStr.split(' ')[3]  || 'N/A' ;

              let usedv  = 0;
              let totalv = 0;
              let usedu  = '';
              let totalu = '';
              if (usedStr.slice(-3) === '无限制') {
                usedu   = '无限制' ;
                totalu  = '无限制' ;
              }else{
              ...
              const ipStr = infoItems[4].querySelector(':scope > div:last-child')?.textContent.trim() || 'N/A';  // IP 
              const ipv4  = ipStr.split(' ')[1] || 'N/A';
              const ipv6  = ipStr.split(' ')[3] || 'N/A';

              const renewStr = infoItems[5].querySelector(':scope > div:last-child')?.textContent.trim() || 'N/A';  // IP 
          ...
          //   console.log(period, renewPrice);
              ...
              const expireStr = infoItems[7].querySelector(':scope > div:last-child')?.textContent.trim() || 'N/A';  // IP 
              // const expire     = infoBodies[8].querySelector('.info-value')?.textContent.trim() || 'N/A';
              const timeDiff   = calculateTimeDifference(expireStr);
          ...
          //   const dayOK = timeDiff.days >= 14 ? '✔️' : '❌';
              ...
              if ( is2Push ) {
                  data.push({ card, name, location,ipstatus, period,node, plan, cpuNum, cpuUnit, mem, diskNum,diskUnit,
                      bandw, usedv, usedu, totalv,totalu, ipv4,ipv6,
                      price,sellPrice, renew,renewPrice, pricetorenew,
                      expire: expireStr, daysleft,hourleft,dayOK: okBuy,isBuy,buyBtn,
                  });
              }
              // console.log('renewPrice:', renewPrice,period, 'daysleft:', daysleft, 'hourleft:', hourleft);
              // return ;
          } catch (error) {
  ...
  function createResultTable(data) {
    ...
    data.forEach(item => {
        ...
        detailCell.textContent = item.ipstatus.slice(-2);
        if (detailCell.textContent === '被墙') {
        ...
        cpuCell.textContent = item.cpuNum.toString()+'核';
        if (item.cpuNum  > 1) {
        ...
        diskCell.textContent = item.diskNum.toString()+'GB';
        ...
        planCell.textContent = item.plan;
        planCell.className = 'td-second';
  ...
  delMoreBtn.addEventListener('click', () => {delLoadMoreBtn();});
  ...
  window.addEventListener("scroll", () => {
    ...
    delMoreBtn.style.display   = shouldShow ? "flex" : "none";

...
install_agent() {
    ...
    if [ -z "$CN" ]; then
        ...
        GITHUB_URL="https://proxy.180102.xyz/proxy/https://github.com"
    fi
    NZ_AGENT_URL="${GITHUB_URL}/nezhahq/agent/releases/download/${_version}/nezha-agent_linux_${os_arch}.zip"

    _cmd="wget -L -t 2 -T 60 -O nezha-agent_linux_${os_arch}.zip $NZ_AGENT_URL >/dev/null 2>&1"
    if ! eval "$_cmd"; then
